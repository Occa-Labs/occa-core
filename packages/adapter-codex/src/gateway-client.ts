// HTTP client for the remote Codex Gateway (BYORT mode). Mirrors the Claude
// Gateway client — runCodex → POST /v1/run, probe → GET /v1/health, plus
// seed/deprovision/cancel — over the SAME wire protocol. The gateway service
// (Phase 2) wraps `codex exec` and normalizes its native `--json` events into
// the runtime-agnostic CodexStreamEvent shape this client reads.
//
// The gateway streams NDJSON for /v1/run: one `{t:"event"}` line per run
// event, a final `{t:"result"}` line carrying the RunCodexResult.
...
function errorResult(error: string, reason: string): RunCodexResult {
...
// and codex is logged in / authed on the remote box.
...
// returning the final RunCodexResult.
//
...
export async function gatewayRun(
  ...
  for (;;) {
    ...
    let buf = "";
    let result: RunCodexResult | null = null;

    const routeLine = (line: string): void => {
      const trimmed = line.trim();
      if (!trimmed) return;
      let parsed: { t?: string; event?: CodexStreamEvent; result?: RunCodexResult };
      try {
