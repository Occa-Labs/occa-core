...
// host note, a verbose claude version — can overflow the right border.
...
export interface BannerInput {
  ...
  version: string;
  claude: { ok: boolean; version?: string; reason?: string };
  /** Show an https:// listening URL (the gateway is serving TLS itself). */
  secure?: boolean;
}

export function printBanner({ config, version, claude, secure }: BannerInput): void {
  const scheme = (secure ?? Boolean(config.tls)) ? "https" : "http";
  const host = config.host ?? "0.0.0.0";
  const hostNote = config.host ? "" : "  (all interfaces)";
  const claudeStatus = claude.ok
    ? paint(ANSI.green, `present  ${claude.version ?? ""}`.trim())
    : paint(ANSI.red, `not found — ${claude.reason ?? "install the claude CLI"}`);

  const lines: string[] = [
    "",
    ...LOGO.map((l) => `  ${paint(ANSI.cyan, l)}`),
    "",
    `  ${paint(ANSI.bold, "OCCA · Claude Gateway")}  ${paint(ANSI.dim, `v${version}`)}`,
    `  ${paint(ANSI.dim, "BYORT runtime that wraps `claude -p` over HTTP.")}`,
    "",
    ...renderBox([
      `  listening   ${scheme}://${host}:${config.port}${hostNote}`,
      `  bearer      configured`,
      `  claude      ${claudeStatus}`,
      `  workspace   ${process.env.OCCA_CLAUDE_WORKSPACE_ROOT ?? "~/.occa-claude-agents"}`,
    ]),
    "",
