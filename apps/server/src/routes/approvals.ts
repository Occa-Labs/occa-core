...
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
  ...
  if (patchEntries.length === 0) {
    // Nothing to do — reply with current row.
...
// Strip server-stamped lifecycle keys from a payload snapshot so the
// `originalPayload` audit field carries only what the agent requested,
// not metadata the system added afterwards.
...
function stripSystemKeys(payload: Record<string, unknown>): Record<string, unknown> {
