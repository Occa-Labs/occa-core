...
export const CEO_ACTIONS: Record<string, CeoActionDescriptor> = {
  ...
  PROPOSE_WORKFLOW_CREATE: {
    ...
    promptSpec: [
      ...
      `- A step can be a review GATE: add "kind: gate" and assign it to "role:head_editorial". The head reviews the prior work and emits a go/fail/kill verdict — go advances, fail returns it to the draft step to revise (capped by "caps.max_revisions", default 2), kill ends the run. Place the gate AFTER the steps it reviews (e.g. draft → verify → gate).`,
      `- A step can be a deterministic TOOL action: add "kind: tool" with "tool" (a company tool type, e.g. publish) and "action" (e.g. post). The engine invokes that tool AS "assigned_to"'s role with the prior step's output — no agent, no tokens. Publishing the finished piece is "kind: tool, tool: publish, action: post, assigned_to: role:head_editorial" as the LAST step.`,
  ...
  PROPOSE_WORKFLOW_EDIT: {
    ...
    promptSpec: [
      ...
      `- A step can be a review GATE: add "kind: gate" and assign "role:head_editorial" — the head emits a go/fail/kill verdict on the prior work (go advances, fail returns to the draft step, kill ends the run). Place it after the steps it reviews.`,
