...
export const CEO_ACTIONS: Record<string, CeoActionDescriptor> = {
  ...
  PROPOSE_ROUTINE_EDIT: {
    ...
    promptSpec: [
      ...
      `- This marker does NOT pause/resume (use TOGGLE_ROUTINE), reassign (use ASSIGN_ROUTINE), run now (use DISPATCH_ROUTINE), or change the schedule/triggers. Use it only for mandate edits + delete.`,
      `- For "update", include at least one of title, description, or priority.`,
      `- After emitting, tell the owner you've queued the change for their approval. Do NOT write a fake "✓" — the runtime appends the receipt.`,
      ``,
  ...
  PROPOSE_SKILL_LIBRARY_EDIT: {
    ...
    promptSpec: [
      `PROPOSE_SKILL_LIBRARY_EDIT MARKER (use when the owner asks to change which roles may use a library skill, or to remove a skill from the company library):`,
      `- This does NOT write. It creates a PROPOSAL the owner reviews and commits in the Approvals window. Until they approve, nothing changes.`,
      `- "op" is "set_roles" (replace the skill's allowed-roles whitelist) or "remove" (delete the skill from the library).`,
      ...
      `- This targets only company-imported skills. You CANNOT edit or remove platform built-in skills, and you CANNOT import a new skill (that is operator-only). Installing/uninstalling a skill onto a specific agent is a different, separate marker.`,
      `- After emitting, tell the owner you've queued the change for their approval. Do NOT write a fake "✓" — the runtime appends the receipt.`,
      ``,
