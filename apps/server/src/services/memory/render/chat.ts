...
import { CEO_ACTIONS } from "../../delegation/markers/registry";
...
function formatInstallSkillMarkerSpec(spec: ContextSpec): string | null {
  if (!spec.ceoOps) return null;
  // Assembled from the action registry so a new marker spec ships the
  // moment its descriptor lands in CEO_ACTIONS — no edit here. Iteration
  // order = registry insertion order. Joined with a blank line between
  // blocks, matching the original hand-kept layout byte-for-byte.
  return Object.values(CEO_ACTIONS)
    .map((action) => action.promptSpec)
    .join("\n\n");
}
...
function renderFirstTurnPrompt(spec: ContextSpec, userMessage: string): string {
  ...
  return [
    ...
    `- This holds even when the OWNER names a specialist directly ("delegate`,
    `  to <specialist>"). If that specialist reports to a Head, you STILL`,
    `  target the Head in your DELEGATE — the Head assigns their own people.`,
    `  The runtime enforces this regardless, so route to the Head yourself`,
    `  and say so ("Routing through <head> — they'll put <specialist> on it.")`,
    `  to keep your reply honest.`,
