...
function renderFirstTurnPrompt(spec: ContextSpec, userMessage: string): string {
  ...
  return [
    ...
    `- If a HEAD owns that domain in your active team → propose them ("I'll have <head> handle this.").`,
    `- If no Head but a specialist/direct-report fits → propose them as a fallback ("No <head_x> deployed yet — I'll route to <specialist> this time.").`,
