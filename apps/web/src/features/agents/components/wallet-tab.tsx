...
type WalletView = "overview" | "invoices" | "transactions" | "onchain";

const WALLET_NAV: { id: WalletView; label: string; icon: React.ReactNode }[] = [
  { id: "overview", label: "Overview", icon: <Wallet className="size-3.5" /> },
  {
    id: "invoices",
    label: "Invoices",
...
export function WalletTab({
  ...
  // Loaded eagerly — the Overview's pending-invoices stat needs the count
  // even before the user opens the Invoices view.
  const invoices = useAgentInvoices(agent.id);

  // Pre-anchor agents have no Deployment PDA on chain yet, so we can't
  // call set_receiving_address. Keep the tab visible (so users see the
  // gating reason) but disable the action.
  const isAnchored = agent.agentChainTxSignature !== null;

  return (
    <div className="flex h-full min-h-0">
      <aside className="w-40 shrink-0 border-r border-white/8 py-4 px-2 flex flex-col gap-0.5">
        {WALLET_NAV.map((item) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 px-2.5 py-1.5 rounded text-xs font-medium text-left transition-colors cursor-pointer ${
                active
      ...
      <div className="flex-1 min-w-0 overflow-y-auto p-5">
        ...
        {view === "onchain" && (
          <OnChainSection data={onchain.data} loading={onchain.isLoading} />
        )}
      </div>
          ...
          <button
            ...
            title={
              canEdit
                ...
                : "Anchor this agent on chain first (Overview tab → anchor panel)"
            }
...
function OnChainSection({
  ...
  if (!data?.deployment && !data?.identity) {
    return (
      <SectionFrame
        icon={<ExternalLink className="size-4" />}
        title="On-chain data"
      >
        <div className="text-xs text-white/40 italic">
          Agent isn&apos;t anchored yet. Use Overview → Anchor on chain to
          register the Deployment + Identity PDAs.
        </div>
      </SectionFrame>
    );
  }
